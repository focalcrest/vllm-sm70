...
class GPUModelRunner(
    ...
    def _get_kv_cache_stride_order(
        ...
        try:
            try:
                # SM70/TQ backends key their physical layout on the cache dtype
                # (turboquant_* caches use a different stride order); pass it
                # through so the shared raw KV tensor is laid out correctly.
                kv_cache_stride_order = attn_backend.get_kv_cache_stride_order(
                    cache_dtype_str=cache_dtype_str
                )
            except TypeError:
                # Standard upstream backends don't accept the cache_dtype_str
                # kwarg; fall back to their plain signature.
                kv_cache_stride_order = attn_backend.get_kv_cache_stride_order()
    ...
    def _get_standard_kv_cache_orders(
        ...
        stride_order = cls._get_kv_cache_stride_order(
            attn_backend, kv_cache_shape, cache_dtype_str
        )
